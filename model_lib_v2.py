    ...
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        for _ in range(global_step.value(), train_steps, num_steps_per_iteration): 
          ...
          current_step = int(global_step.value())
          ...
          # 09.30
          if int(global_step.value()) >= int(train_steps):
            tf.logging.info(f"[FINAL STEP] :{int(global_step.value())} (ensure final log)")          
          # 06.11 추가
          ...
          # if global_step.value() - logged_step >= num_steps_per_iteration:  # 09.30_주석처리
          if int(global_step.numpy()) % int(num_steps_per_iteration) == 0:  # 09.30_교체
            logged_dict_np = {name: value.numpy() for name, value in logged_dict.items()}
          ...
          # 마지막 체크포인트 저장 루틴_05.29 (global_step값 이상이면 무조건 저장)
            ...
            if global_step >= early_start:	# 09.29 변경
